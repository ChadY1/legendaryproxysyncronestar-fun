...
2. Fournis les artefacts (mods/plugins/maps/assets GUI) via un miroir interne ou un stockage S3 ; le script consomme `MOD_MIRROR_BASE`, `PLUGIN_MIRROR_BASE`, `MAP_MIRROR_BASE` et `GUI_ASSET_BASE` pour télécharger automatiquement les fichiers listés dans `mods.list`, `plugins.list`, `maps.list` et `assets/gui.list` si tu ajoutes les URLs. Plusieurs URLs publiques sont pré-renseignées (Skript, LuckPerms, Flan's packs, TooManyItems, SecurityCraft, textures StarLife, etc.) pour faciliter les tests connectés. Les fichiers existants sont ré-utilisés et ré-extraits à chaque passage. La section "Bundle iRP" de `plugins.list` pointe directement vers les binaires du dépôt Rubylium/iRP (Vault, WorldGuard, Essentials, TuSKe, WildSkript, Umbaska, Citizens...) afin d'obtenir les mêmes dépendances que les scripts importés.
...
6. Pour les GUIs roleplay (ATM, pompe, concessionnaire), ajoute les textures PNG dans `proxy/assets/gui` en suivant `proxy/assets/gui/README.md`, puis copie les scripts SQript du dossier `proxy/sqript/` dans `mods/SQript/scripts`. Les nouveaux scripts `starliferp_core.sq` (chat RP/HRP, menottes, PV électroniques) et `starliferp_jobs.sq` (ressources/atelier) reprennent l'esprit iRP avec des commandes renommées en `/slr...` et des messages StarLifeRP.
