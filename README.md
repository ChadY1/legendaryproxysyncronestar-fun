...
- Conseils de dépannage : si un serveur refuse la commande, vérifie les permissions `/slr` et le chargement du mod dans les logs FML (`starliferp` doit apparaître sans exception). La version actuelle embarque aussi des ponts SRG (`func_71517_b`, `func_71515_b`, etc.) pour Mohist/Forge 1.7.10 afin d'éviter tout `AbstractMethodError` sur `registerCommand` même sans reobfuscation ForgeGradle.
