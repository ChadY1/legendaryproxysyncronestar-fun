...
- Conseils de dépannage : si un serveur refuse la commande, vérifie les permissions `/slr` et le chargement du mod dans les logs FML (`starliferp` doit apparaître sans exception). Un `AbstractMethodError` sur `registerCommand` indique que le mod n'était pas basé sur `CommandBase` : mets à jour le JAR avec la version courante pour aligner la signature Forge 1.7.10.
